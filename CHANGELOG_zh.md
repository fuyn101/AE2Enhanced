...
- **放置工具系统重做**
  - 新增独立物品 **ME 放置工具（ME Placement Tool）**：通过 AE2 安全终端绑定网络，可直接放置方块、AE2 Part、Facade 与线缆
  - **先进 ME 全能工具**移除扳手兼容模式，替换为放置模式
  - 三种放置子模式：单格放置 / 批量放置 / 线缆放置
  - **批量放置（建筑手杖风格）**：
    - 右键已有方块的面，按该面法向整片延伸一层
    - 支持方向锁：水平 / 竖直 / 南北 / 东西 / 无限制
    - 最大 512 格，失败自动回滚，Ctrl+右键撤销最近一次放置
  - **线缆放置**：
    - 右键设置起点，再次右键设置终点，沿曼哈顿最短路径自动铺设
    - C 键配置 GUI 选择线缆颜色（16 色 + Fluix）
...
- **ProjectE EMC 接口**
  ...
  - 复用 3.png 纹理图集配套布局：每页 8×4 过滤格，共 20 页；中下部显示玩家背包与快捷栏，左右翻页按钮使用 3.png 按钮 UV
  - 白名单槽仅允许存在 EMC 值的物品，空手点击已标记槽位可取消标记
