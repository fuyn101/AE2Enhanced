 ...
 * 继承 ContainerUpgradeable，复用过滤槽和升级槽布局。
 */
public class ContainerUniversalImportBus extends ContainerUpgradeable implements IOptionalSlotHost {

    private final PartUniversalImportBus part;
    ...
    private void setSchedulingMode(SchedulingMode mode) {
            ...
            if (s instanceof SlotFake) {
                ...
                if (!held.isEmpty()) {
                    ...
                    if (fake != null && !fake.isEmpty()) {
                        ...
                        // 右键（SPLIT_OR_PLACE_SINGLE）及所有其他 action 保持 AE2 原生逻辑，
                        // 放入物品本身（不转换）
                    }
                }
            }
    ...
    /**
     * 尝试将手持物品转换为假物品（流体/气体/源质）。
     * 返回 null 表示不是可转换的容器，由原生逻辑处理。
     */
